...
{
    if (read_file->IsOpen())
        std::cout << "ROOT file opened successfully\n"
                  << std::endl;
    for (unsigned int i = 0; i < N_E_BINS; i++)
    {
...
void Histogram::set_cnt_hist(TH1F *_cnt_hist)
{
    count_hist = (TH1F *)_cnt_hist->Clone();
};

void Histogram::set_flx_hist(TH1F *_flx_hist)
{
    flux_hist = (TH1F *)_flx_hist->Clone();
};

void compute_multi_region_flux(Histogram *_histogram, std::vector<NADIR_BOUND> _nadir_bounds)
{
    auto cnt_maps = _histogram->get_cnt_maps();
    auto exp_maps = _histogram->get_exp_maps();

    float solid_angle_limb = 0.0f;
    for (NADIR_BOUND nb : _nadir_bounds)
    {
        solid_angle_limb += Transform::getSolidAngle(
            nb.phi_lower, nb.phi_upper, nb.theta_lower, nb.theta_upper);
    }
    auto solid_angle_bg = Transform::getSolidAngle(
        PHI_NADIR_MIN, PHI_NADIR_MAX,
        THETA_NAD_BG_MIN, THETA_NAD_BG_MAX);
    ...
    int n_photon_limb;
    float flxmap_val_i;
    auto new_count_hist = _histogram->get_cnt_hist();
    auto new_flx_hist = _histogram->get_flx_hist();
    auto energy_bin_edges = _histogram->get_energy_edge_bins();
    for (unsigned int i_energy_bin = 0; i_energy_bin < N_E_BINS; i_energy_bin++)
    {
        auto dE = energy_bin_edges[i_energy_bin + 1] - energy_bin_edges[i_energy_bin];

        auto flx_map = (TH2F *)cnt_maps[i_energy_bin]->Clone();
        flx_map->Divide(exp_maps[i_energy_bin]);
        // new count hist
        n_photon_limb = 0;
        flxmap_val_i = 0.0f;
        for (NADIR_BOUND nb : _nadir_bounds)
        {
            n_photon_limb += Histogram::sumOverRegion(
                cnt_maps[i_energy_bin],
                nb.phi_lower, nb.phi_upper, nb.theta_lower, nb.theta_upper);
            flxmap_val_i += Histogram::sumOverRegion(
                flx_map,
                nb.phi_lower, nb.phi_upper, nb.theta_lower, nb.theta_upper);
        }
        new_count_hist->SetBinContent(i_energy_bin + 1, n_photon_limb);
        new_count_hist->SetBinError(i_energy_bin + 1, sqrt(n_photon_limb));
        ...
        new_flx_hist->SetBinError(
            i_energy_bin + 1,
            new_count_hist->GetBinError(i_energy_bin + 1) * (new_flx_hist->GetBinContent(i_energy_bin + 1) / new_count_hist->GetBinContent(i_energy_bin + 1)));
    }
    _histogram->set_cnt_hist(new_count_hist);
    _histogram->set_flx_hist(new_flx_hist);
}