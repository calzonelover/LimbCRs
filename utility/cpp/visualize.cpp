    ...
    std::string y_label)
{
    ...
    strcpy(_hist_name, hist_name.c_str());
    strcpy(_hist_title, hist_title.c_str());
    strcpy(_file_name, file_name.c_str());
    strcpy(_plot_mode, plot_mode.c_str());
...
void Visualize::plot2DHist(
    ...
    float theta_nadir_min, float theta_nadir_max)
{
    char _map_name[map_name.size() + 1], _map_title[map_title.size() + 1];
    char _file_name[file_name.size() + 1], _plot_mode[plot_mode.size() + 1];
    char _z_label[z_label.size() + 1];
    strcpy(_map_name, map_name.c_str());
    strcpy(_map_title, map_title.c_str());
    strcpy(_file_name, file_name.c_str());
    strcpy(_plot_mode, plot_mode.c_str());
    ...
    float theta_nadir_min, float theta_nadir_max)
{
    ...
    for (unsigned int i = 1; i <= 4; i++)
    {
        ...
        maps[j]->GetXaxis()->SetTitle("#phi_{nadir} (degree)");
