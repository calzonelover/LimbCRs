...
bin width.
% The count map be divided by the the exposure map and the summation 
% has taken all the pixel over limb's region.
% The quantity $\text{Count}_i$ and $\text{Exposure}_i$ 
...
The flux calculation procedure can be summarized in the following steps.
\begin{enumerate}
    \item Analyze 50 bins in energy
    between 10 GeV - 1 TeV with equal logarithmic spacing 
    % with equal logarithmic spacing between 10 GeV - 1 TeV  
...
%     and lifetime of the LAT as it observed the Earth
...
% proton power-law spectrum in rigidity
...
% proton-proton collision with a thin-target 
% where the scaling of normalization depending
...
% mostly consists of nitrogen as the majority and the oxygen
\citep{atmosCompos}.
% The cross-section from 
% proton-proton collision and proton-nitrogen collision is roughly 
% approximate as the multiplicative relation since there is the plateau 
% curve in the energy range in this analysis.
The relation of the proton-proton 
and proton-air scattering amplitudes
% scattering amplitude and the proton-air
is obtained from \cite{WAtwater}
which depends on the
% where it highly depends on the
atomic numbers of the air molecules. 


The second highest fraction of the arrival CR particles on 
...
% The helium nuclei can interact with the nitrogen and produce high energy $\gamma$-rays depends on
% how much kinetic energy $\alpha$-particles is holding.
...
% the same relationship of the proton-air collision model.
Hence,
the $\gamma$-ray spectrum produced from CR proton and
$\alpha$ scattering with air molecules can be described by
% derived equation of the scattering process from incoming 
...
\cite{K&Omodel}. % Kachxxx&Ostapxx (20xx).
The CR
$\alpha$ spectrum ($dN_\alpha/dR$) % helium spectrum
...
% The spectrum of $\alpha$-particles is directly measured 
% by \cite{AMS-02Helium} and fix the distribution as one function rather 
% than modeled spectrum of CR proton spectrum.
...
% A proper methodology for fitting or optimization in another word is also 
...
% the model spectrum to be
a count histogram is simply
multiplying the model flux and the exposure
% multiplied by the integral of the exposure map
in the limb region.
...
% real measurement. Not only for numerical convenience but the negative sign 
...
% parameters, the different best-fit model 
...
In the next iteration, we slowly move those particles % Then rest of them would slowly move
...
% As the language of statistics 
% would consider as two hypotheses. One for null and one for an alternative 
% approach. 
...
% in the previous paragraph, the significance level has to be taken
% into account to put the weight on the study.
...
To compare the null (SPL) and alternative (BPL) hypothesis,
...
% hypothesis is similar to one-tail hypothesis testing with a given 
% dependency in more DOF has been
