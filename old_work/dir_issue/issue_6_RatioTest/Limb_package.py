...
class deal_simulation:
    def __init__(self,count_bin, Eavgbin, flux_sim):
        ...
        self.SLPS = None # Sum Log Poisson
    def SumlogPois(self, dummy): # g_sim = TGraph from simulation
        ...
        for i in range(len(E_model)):
            ...
            # print "Halo"
            # print count_i_measurement ###
            # print count_i_model ###
            # exit() ###
            if Math.poisson_pdf(int(count_i_model),count_i_measurement) == 0:
                sumlogpois += 308.
            if Math.poisson_pdf(int(count_i_model),count_i_measurement) != 0:
                sumlogpois += -log(Math.poisson_pdf(int(count_i_model),count_i_measurement))
                # sumlogpois += -log(TMath.Poisson(count_i_measurement,count_i_model))
        self.SLPS = sumlogpois
        return sumlogpois
